    ...
    const hiringProcessSteps: IHiringProcessStep[] = [
        {
            icon: () => <svg width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect width="60" height="60" rx="6" fill="#10B981" />
                <path d="M31.72 38H28.94V26.88H26.38V24.48H31.72V38Z" fill="white" />
            </svg>,
            title: dictionary.pages.careers.hiring_process.values[0].title,
            description: dictionary.pages.careers.hiring_process.values[0].description,
        },
        {
            icon: () => <svg width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect width="60" height="60" rx="6" fill="#10B981" />
            ...
            title: dictionary.pages.careers.hiring_process.values[1].title,
            description: dictionary.pages.careers.hiring_process.values[1].description,
        }, {
            icon: () => <svg width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect width="60" height="60" rx="6" fill="#10B981" />
            ...
            title: dictionary.pages.careers.hiring_process.values[2].title,
            description: dictionary.pages.careers.hiring_process.values[2].description,
        ...
        <WebSection about="Hiring process" className="py-16  bg-white my-16 text-white">
            <div className="bg-brand-blackblue md:p-10 msm:p-4 flex flex-col gap-10  rounded-lg">
                <div className={` flex flex-col  pb-10 text-center flex flex-col items-center mx-auto border-b-2 w-fit  border-brand-white-10 `}>
                    <Text variant={"suptitle"} className="text-brand-white italic font-semibold text-sm" >
                        {dictionary.pages.careers.hiring_process?.suptitle || "HIRING PROCESS"}
                    </Text>
                    <Text variant={"title_2"} className="text-brand-white">
                        {dictionary.pages.careers.hiring_process?.title || "How We Hire"}
                    </Text>
                    <Text variant={"subtitle"} className="text-brand-white">
                        {dictionary.pages.careers.hiring_process.description}
                    </Text>
                </div>
                <div className="grid md:grid-cols-3 msm:grid-cols-1 gap-6">
                    {
                        hiringProcessSteps.map((step, index) => (
                            <div className="flex flex-col w-full p-10 bg-brand-white-10 gap-3 rounded-md">
                                <step.icon />
                                <Text variant="heading3" className="text-white"> {step.title}</Text>
                                <Text className="text-white"> {step.description}</Text>
                            </div>
                        ))
                    }
                </div>
